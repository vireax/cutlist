...
//			$('#kerf').append(el2);
			...
			for(var i = 0; i < s.cut_list.length; i++){
				...
				var el = $(sprintf("<li><a href='#'><span class='l6em'>%s </span> &times; %s</a></li>", s.cut_list[i][1], s.cut_list[i][0]));
			...
			for(var i = 0; i < s.stock_array.length; i++){
				var display_size = 95 * (s.stock_array[i].size/max_stock_size);
				var el = $(sprintf("<div class='bin-outer'><div class='bin-label'>%s (%s)</div><div class='bin-inner' style='height: %s%%'></div></div>", i+1, s.stock_array[i].size, display_size));
				$('#floor').append(el);
				...
				var waste = s.stock_array[i].size - s.kerf;
				for(var j = 0; j < cuts.length; j++){
					var cut_size = Math.floor((inner_el.width()-1) * (cuts[j] / s.stock_array[i].size)) - 1;
					/*var cut_el = $(sprintf("<div class='parcel' style='width: %spx; line-height: %spx'>%s</div>", cut_size, cut_size, cuts[j]));*/
					waste -= cuts[j]+s.kerf;
					var cut_el = $(sprintf("<div class='parcel' style='width: %spx;'>%s</div>", cut_size-15, cuts[j]));
					
					inner_el.append(cut_el);
				}
				var waste_percent = waste*100/s.stock_array[i].size;
				var waste_el = $(sprintf("<div class='remain'>%s (%s &#37;)</div>",  waste, waste_percent.toFixed(2)));
				inner_el.append(waste_el);
			}
